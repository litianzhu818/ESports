...
@interface StrengScoreTeamFifthCell () <PNChartDelegate>
...
@implementation StrengScoreTeamFifthCell
...
- (void)layoutSubviews
{
    ...
    self.priceChangeValueLabel.text = [NSString stringWithFormat:@"%ld",labs(priceChange)];
    if (priceChange == 0) {
    ...
    for (NSInteger index = 0; index < 6; ++index) {
        [yLabelValues addObject:[NSString stringWithFormat:@"%ld",(unsigned long)(self.lineChart.yFixedValueMin + index * eachMinInterval)]];
    }
...
+ (UINib *)nib
{
    return [UINib nibWithNibName:NSStringFromClass([StrengScoreTeamFifthCell class])
                          bundle:[NSBundle bundleForClass:[StrengScoreTeamFifthCell class]]];
}

+ (NSString *)cellIdentifier
{
    return NSStringFromClass([StrengScoreTeamFifthCell class]);
}
